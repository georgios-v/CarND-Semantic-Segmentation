...
def load_vgg(sess, vgg_path):
    ...
    t_dropout = graph.get_tensor_by_name(vgg_keep_prob_tensor_name)
    t_l3 = graph.get_tensor_by_name(vgg_layer3_out_tensor_name)
    t_l4 = graph.get_tensor_by_name(vgg_layer4_out_tensor_name)
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output = tf.layers.conv2d_transpose(output, num_classes, 4, strides=(2, 2),
                                    ...
                                    kernel_initializer= tf.random_normal_initializer(stddev=0.01),
                                    kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    conv1x1 = tf.layers.conv2d(vgg_layer4_out, num_classes, 1, strides=(1, 1),
                                    padding='same',
                                    kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                    kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output = tf.add(output, conv1x1)

    output = tf.layers.conv2d_transpose(output, num_classes, 4, strides=(2, 2),
                                    padding='same',
                                    kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                    kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    conv1x1 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, strides=(1, 1),
                                    padding='same',
                                    kernel_initializer=tf.random_normal_initializer(stddev=0.01),
                                    kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    output = tf.add(output, conv1x1)
...
def run():
    ...
    with tf.Session() as sess:
        ...
        helper.save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, t_dropout, t_input)
